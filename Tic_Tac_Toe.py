...
# See if there is a win from that last move (either from the User or Computer)
def lookForWin(grid):
    ...
    if grid[0][0] == grid[0][1] and grid[0][1] == grid[0][2] and grid[0][0] != ".":
        ...
        return (
            "Congratulations",
            grid[0][0],
            " you got three in the top left diagonal!",
        )
    elif grid[0][2] == grid[1][1] and grid[0][2] == grid[2][0] and grid[0][2] != ".":
        return (
            "Congratulations",
            grid[0][2],
            " you got three in the bottom left diagonal!",
        )
...
def isGridComplete(grid):
    assert (
        len(grid) == 3 and len(grid[0]) == 3 and len(grid[1]) == 3 and len(grid[2]) == 3
    )
...
# Mpresley 6/30/23 the computer will move
def ComputerMove(grid, comp_symbol):
    ...
    if col1.count(".") == 1 and col1.count(comp_symbol) == 2:
        ...
        grid[0][1] = comp_symbol
        grid[1][1] = comp_symbol
        grid[2][1] = comp_symbol
        print(lookForWin(grid))
        print("col hit 2 ")
        return
